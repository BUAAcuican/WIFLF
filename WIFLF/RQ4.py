...
def WeightedIsolationForestwithLabelinformationFilter(mode, clf_index, sp, runtimes):
    pwd = os.getcwd()
    print(pwd)
                ...
                if train_mode == 'M2O_CPDP':  # train data contains more files different from the test data project
                    ...
                    Sample_tr_pos, Sample_tr_neg, Sample_pos_index, Sample_neg_index \
                        = Random_Stratified_Sample_fraction(Samples_tr_all, string='bug',
                                                            r=r, typicalFracDict={1: sp, 0: sp})  # random sample 90% negative samples and 90% positive samples
    ...
    pathname = spath + '\\' + (save_file_name + '_' + modelname[0] + '_' + str(sp) + '.csv')
    df_file_measures.to_csv(pathname)
    # print('df_file_measures:\n', df_file_measures)
    return df_file_measures

def AdjustSamplePercentofWIFLF():
    pwd = os.getcwd()
    # print(pwd)
    father_path = os.path.abspath(os.path.dirname(pwd) + os.path.sep + ".")
    # print(father_path)
    # datapath = father_path + '/dataset-inOne/'
    spath = father_path + '/spResults/'
    if not os.path.exists(spath):
        os.mkdir(spath)
